...
public class ToDoRestController {
    ...
    boolean setDone(@PathVariable String mid, @PathVariable Long seq) {
        todoService.setDone(seq);
        return true;
    }

    // /* --- U: ToDoを更新する --- */
    @PutMapping("/{mid}/todos/{seq}")
    ToDo updateToDo(@PathVariable Long seq, @Validated @RequestBody ToDoForm form) {
        return todoService.updateToDo(seq, form);
    }

    // /* --- D: ToDoを削除する --- */
    @DeleteMapping("/{mid}/todos/{seq}")
    boolean deleteToDo(@PathVariable Long seq) {
